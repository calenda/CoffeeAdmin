...
public class HoyActivity extends ListActivity {
	...
	private ComandaListAdapter adapter;
	private int mYear;
	private int mMonth;
	private int mDay;
	...
	private void updateDisplay() {
		this.mDateDisplay.setText(DateFormat.format("dd MMMM yyyy",
	...
	public Double totalHoy() {
		Double total = 0.0;
		if (fechaSelected.getTime().compareTo(cHoy.getTime()) == 0) {
			comandas = (ArrayList<Comanda>) DatabaseManager.getInstance()
					.getAllComandasToday();
			// Toast.makeText(this, "IGUALES", Toast.LENGTH_SHORT).show();
		} else {
			comandas = (ArrayList<Comanda>) DatabaseManager.getInstance()
					.getAllComandasByDay(fechaSelected);
